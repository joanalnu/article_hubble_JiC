...
for i in range(len(sorted_I_dis_data)):
    ...
    #converted_sorted_I_dis_low.append(sorted_I_dis_low[i] / 1000000)
    #converted_sorted_I_dis_high.append(sorted_I_dis_high[i] / 1000000)
...
#for value in converted_sorted_vel_data:
    #print(value)
#print("")
#for value in converted_sorted_V_dis_low:
    #print(value)
#print("")
#for value in converted_sorted_I_dis_high:
    #print(value)
...
low_fit_linear_regression = (slope-slope_low) * x + intercept

slope_high, intercept_high = np.polyfit(x_high, y_high, 1)
high_fit_linear_regression = abs(slope+slope_high) * x + intercept

ax.plot(x, fit_linear_regression, color='red')
ax.plot(x, low_fit_linear_regression, color='orange', linestyle='dotted')
ax.plot(x, high_fit_linear_regression, color='orange', linestyle='dotted')



print('slope:', slope)
print('intercept:', intercept)
print('Error low (slope): ', slope_low, ' (', slope-slope_low, ')', sep="")
print('Error high (slope): ', slope_high, ' (', slope+slope_high, ')', sep="")
print(slope+intercept)
labels = 'Hubble costant: ' + str(slope) + ' ±' + str(slope_high) + ' km s^-1 Mpc^-1'
xmin, xmax = plt.xlim()
ymin, ymax = plt.ylim()

plt.text(xmax * 0.95, ymax * 0.98, labels, fontsize=20, ha='right', va='top')
...
axs.set_ylabel('distance V band [pc]')
#axs.set_title('fig. 1a')
axs.grid(False)
axs.errorbar(aux_id, V_dis_data, yerr=[V_dis_low, V_dis_high], fmt='none', color='gray', capsize=5)
#plt.xticks(ticks=np.arange((len(aux_id))), fontsize=5)
...
axs.set_xlabel('Galaxy ID')
axs.set_ylabel('distance I band [pc]')
#axs.set_title("fig. 1b")
axs.grid(False)
axs.errorbar(aux_id, I_dis_data, yerr=[I_dis_low, I_dis_high], fmt='none', color='gray', capsize=5)
#plt.xticks(ticks=np.arange((len(aux_id))), fontsize=6)
...
#axs[1].scatter(aux_id, this_velocities_data)
axs[0].set_xlabel('Galaxy ID')
axs[0].set_ylabel('velocities [m/s]')
...
axs[0].set_ylim(bottom=2600000, top=None, emit=True, auto=False, ymin=None, ymax=None)

axs[1].scatter(aux_id, this_velocities_data, color="tab:blue")
#axs[1].scatter(aux_id, this_velocities_data)
axs[1].set_xlabel('Galaxy ID')
axs[1].set_ylabel('velocities [m/s]')
...
axs[1].set_ylim(bottom=-500000, top=2600000, emit=True, auto=False, ymin=None, ymax=None)
